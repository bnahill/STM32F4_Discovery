...
static const i2c_config_t i2c1_config = {
	GPIO_AF_I2C1,
	// SDA
	{GPIOB, BIT(9), GPIO_PinSource9},
	// SCL
	{GPIOB, BIT(8), GPIO_PinSource8},
	// IRQs
	I2C1_EV_IRQn,
	I2C1_ER_IRQn,
	// Clock
...
static const i2c_config_t i2c2_config = {
	GPIO_AF_I2C2,
	// SDA
	{GPIOB, BIT(11), GPIO_PinSource11},
	// SCL
	{GPIOB, BIT(10), GPIO_PinSource10},
	// IRQs
	.irq_ev = I2C2_EV_IRQn,
	.irq_er = I2C2_ER_IRQn,
	// Clock
...
static const i2c_config_t i2c3_config = {
	GPIO_AF_I2C3,
	// SDA
	{GPIOC, BIT(9), GPIO_PinSource9},
	// SCL
	{GPIOC, BIT(8), GPIO_PinSource8},
	// IRQs
	I2C3_EV_IRQn,
	I2C3_ER_IRQn,
	// Clock
...
int i2c_init(i2c_t *i2c, i2c_mode_t mode, uint32_t speed){
	...
	GPIO_PinAFConfig(conf->sda.gpio, conf->sda.pinsrc, conf->af);
	GPIO_PinAFConfig(conf->scl.gpio, conf->scl.pinsrc, conf->af);
/*
	// Mode configuration
...
*/
	...
	gpio_init_s.GPIO_Pin = conf->scl.pin;
	GPIO_Init(conf->scl.gpio, &gpio_init_s);
	gpio_init_s.GPIO_Pin = conf->sda.pin;
	GPIO_Init(conf->sda.gpio, &gpio_init_s);
...
void i2c_write_byte(i2c_t *i2c, uint8_t devaddr, uint8_t addr, uint8_t value){
	i2c_transfer_t xfer;
	i2c_mk_transfer(&xfer, I2C_OP_WRITE, devaddr, addr, &value, 1);
...
uint8_t i2c_read_byte(i2c_t *i2c, uint8_t devaddr, uint8_t addr){
	uint8_t result;
	i2c_transfer_t xfer;
	i2c_mk_transfer(&xfer, I2C_OP_READ, devaddr, addr, &result, 1);
	i2c_transfer(i2c, &xfer);
	while(!xfer.done);
	return result;
}
